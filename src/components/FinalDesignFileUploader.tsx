...
import { Upload, X, Eye, Save, Loader2, Check, FileText, Image, Download, AlertCircle, Link, ExternalLink } from 'lucide-react';
...
interface FinalDesignFileUploaderProps {
  project: Project;
  onSave: (projectId: string, files: AirtableAttachment[]) => Promise<boolean>;
  isReadOnly?: boolean;
}
...
const FinalDesignFileUploader: React.FC<FinalDesignFileUploaderProps> = ({
  ...
  // URL field state
  const [designFileUrl, setDesignFileUrl] = useState(project.finalDesignFileLink || '');
  const [isUrlSaving, setIsUrlSaving] = useState(false);
  const [urlSaveStatus, setUrlSaveStatus] = useState<'idle' | 'success' | 'error'>('idle');
  ...
  const validateFile = (file: File): string | null => {
    ...
    const allowedExtensions = [
      ...
      '.ai', '.psd', '.sketch', '.fig', '.eps', '.indd', '.tiff', '.tif'
    ];
  ...
  const processFileForAirtable = async (file: File): Promise<any> => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = () => {
        try {
          ...
          // Return the format that Airtable expects
          resolve({
            filename: file.name,
  ...
  const handleFileUpload = async (files: File[]) => {
    ...
    try {
      ...
      if (!success) {
        throw new Error('Failed to save files to database. Please check your file formats and try again.');
      }
  ...
  const handleUrlSave = async () => {
    ...
    try {
      const normalizedUrl = designFileUrl.trim() ? normalizeUrl(designFileUrl.trim()) : '';
  ...
  const isImage = (file: AirtableAttachment) => {
    return file.type?.startsWith('image/') || file.filename.match(/\.(jpg|jpeg|png|gif|webp|svg)$/i);
  };
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white p-6 rounded-lg border border-slate-200 space-y-6">
        <div className="flex items-center space-x-2">
          ...
          <h4 className="text-lg font-semibold text-slate-900">Upload Final Design Files</h4>
        </div>
        
        <p className="text-sm text-slate-600">
          Upload the final design files that are ready for production (AI, PSD, PDF, or image files).
        </p>
      ...
      <div className="bg-white p-6 rounded-lg border border-slate-200 space-y-4">
        <div className="flex items-center space-x-2">
          ...
          <h4 className="text-lg font-semibold text-slate-900">Final Design File Link</h4>
        </div>
        ...
        {project.finalDesignFileLink && !hasUrlChanges && (
          <div className="flex items-center space-x-2 text-sm text-green-700 bg-green-50 p-3 rounded-md border border-green-200">
            ...
            >
              <span className="truncate max-w-xs">{project.finalDesignFileLink}</span>
